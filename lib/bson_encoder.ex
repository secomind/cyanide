...
defmodule Bson.Encoder do
  ...
  defimpl Cyanide.Encoder, for: Integer do
    @doc """
    iex> Cyanide.Encoder.encode(2)
    {<<16>>, <<2, 0, 0, 0>>}
    iex> Cyanide.Encoder.encode(-2)
    {<<16>>, <<254, 255, 255, 255>>}
    iex> Cyanide.Encoder.encode -0x80000001
  ...
  defimpl Cyanide.Encoder, for: Float do
    @doc """
    iex> Cyanide.Encoder.encode(1.1)
    {<<1>>, <<154, 153, 153, 153, 153, 153, 241, 63>>}
    """
    def encode(f), do: {<<0x01>>, <<(f)::size(64)-float-little>>}
  end

  defimpl Cyanide.Encoder, for: Atom do
    @doc """
    iex> Cyanide.Encoder.encode(true)
    {<<8>>, <<1>>}
    iex> Cyanide.Encoder.encode(nil)
    {<<10>>, <<>>}
    iex> Cyanide.Encoder.encode(:max_key)
    {<<127>>, <<>>}
    iex> Cyanide.Encoder.encode(:min_key)
    {<<255>>, <<>>}
    iex> Cyanide.Encoder.encode(:nan)
    {<<1>>, <<0, 0, 0, 0, 0, 0, 248, 127>>}
    iex> Cyanide.Encoder.encode(:'+inf')
    {<<1>>, <<0, 0, 0, 0, 0, 0, 240, 127>>}
    iex> Cyanide.Encoder.encode(:'-inf')
    {<<1>>, <<0, 0, 0, 0, 0, 0, 240, 255>>}
    iex> Cyanide.Encoder.encode(:atom)
    {<<14>>, [<<5, 0, 0, 0>>, "atom", <<0>>]}
  ...
  defimpl Cyanide.Encoder, for: Bson.UTC do
    @doc """
    iex> Cyanide.Encoder.encode(Bson.UTC.from_now({1390, 324703, 518471}))
  ...
  defimpl Cyanide.Encoder, for: Bson.Regex do
    @doc """
    iex> Cyanide.Encoder.encode(%Bson.Regex{pattern: "p", opts: "i"})
    {<<11>>, ["p", <<0>>, "i", <<0>>]}
    """
  ...
  defimpl Cyanide.Encoder, for: Bson.ObjectId do
    @doc """
    iex> Cyanide.Encoder.encode(%Bson.ObjectId{oid: <<0xFF>>})
    {<<0x07>>, <<255>>}

    iex> Cyanide.Encoder.encode(%Bson.ObjectId{oid: 123})
  ...
  defimpl Cyanide.Encoder, for: Bson.JS do
    @doc """
    iex> Cyanide.Encoder.encode(%Bson.JS{code: "1+1;"})
    {<<13>>, [<<5, 0, 0, 0>>, "1+1;", <<0>>]}
    iex> Cyanide.Encoder.encode(%Bson.JS{code: "1+1;", scope: %{a: 0, b: "c"}})
  ...
  defimpl Cyanide.Encoder, for: Bson.Bin do
    @doc """
    iex> Cyanide.Encoder.encode(%Bson.Bin{bin: "e", subtype: Bson.Bin.subtyx(:user)})
    {<<5>>,[<<1, 0, 0, 0>>, 128, "e"]}
    """
  ...
  defimpl Cyanide.Encoder, for: Bson.Timestamp do
    @doc """
    iex> Cyanide.Encoder.encode(%Bson.Timestamp{inc: 1, ts: 2})
    {<<17>>,<<1, 0, 0, 0, 2, 0, 0, 0>>}
    """
  ...
  defimpl Cyanide.Encoder, for: BitString do
    @doc """
    iex> Cyanide.Encoder.encode("a")
    {<<2>>, [<<2, 0, 0, 0>>, "a", <<0>>]}
    """
  ...
  defimpl Cyanide.Encoder, for: [Map, HashDict, Keyword] do
    @doc """
    # Map
    iex> Cyanide.Encoder.encode(%{})
    {<<3>>,<<5, 0, 0, 0, 0>>}
    iex> Cyanide.Encoder.encode(%{a: "r"})
    {<<3>>,<<14, 0, 0, 0, 2, 97, 0, 2, 0, 0, 0, 114, 0, 0>>}
    iex> Cyanide.Encoder.encode(%{a: 1, b: 5})
    {<<3>>,<<19, 0, 0, 0, 16, 97, 0, 1, 0, 0, 0, 16, 98, 0, 5, 0, 0, 0, 0>>}
    iex> Cyanide.Encoder.encode(%{a: 1, b: %{c: 3}})
    {<<3>>,<<27, 0, 0, 0, 16, 97, 0, 1, 0, 0, 0, 3, 98, 0, 12, 0, 0, 0, 16, 99, 0, 3, 0, 0, 0, 0, 0>>}

    # HashDict
    iex> Cyanide.Encoder.encode(%HashDict{})
    {<<3>>,<<5, 0, 0, 0, 0>>}
    iex> Cyanide.Encoder.encode(HashDict.put(%HashDict{}, :a, "r"))
    {<<3>>,<<14, 0, 0, 0, 2, 97, 0, 2, 0, 0, 0, 114, 0, 0>>}

    iex> Cyanide.Encoder.encode(%{a: "va", b: "vb", u: %Bson.UTC{ms: "e"}})
    %Bson.Encoder.Error{
      what: ["u", Bson.UTC],
      term: %Bson.UTC{ms: "e"},
      acc: [[[<<2>>, "a", <<0>>, [<<3, 0, 0, 0>>, "va", <<0>>]],
             [<<2>>, "b", <<0>>, [<<3, 0, 0, 0>>, "vb", <<0>>]]]]}
    iex> Cyanide.Encoder.encode([1, 2, %Bson.UTC{ms: "e"}])
    %Bson.Encoder.Error{
      what: ["2", Bson.UTC],
      term: %Bson.UTC{ms: "e"},
      acc: [[[<<16>>, "0", <<0>>, <<1, 0, 0, 0>>],
             [<<16>>, "1", <<0>>, <<2, 0, 0, 0>>]]]}
    iex> Cyanide.Encoder.encode(%{a: "va", b: "vb", c: %{c1: "vc1", cu: %Bson.UTC{ms: "e"}}})
    %Bson.Encoder.Error{
      what: ["c", "cu", Bson.UTC],
      term: %Bson.UTC{ms: "e"},
      acc: [[[<<2>>, "a", <<0>>, [<<3, 0, 0, 0>>, "va", <<0>>]],
    ...
    iex> Cyanide.Encoder.encode(%{a: "va", b: "vb", c: ["c0", %Bson.UTC{ms: "e"}]})
    %Bson.Encoder.Error{
      what: ["c", "1", Bson.UTC],
      term: %Bson.UTC{ms: "e"},
      acc: [[[<<2>>, "a", <<0>>, [<<3, 0, 0, 0>>, "va", <<0>>]],
  ...
  def element(name, value) do
    case Cyanide.Encoder.encode(value) do
